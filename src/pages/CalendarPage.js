...
function CalendarPage() {
  ...
  useEffect(() => {
    const loadModels = async () => {
      ...
      modelRef.current = await faceLandmarksDetection.createDetector(faceLandmarksDetection.SupportedModels.MediaPipeFaceMesh, modelConfig);
  
      // 수정: 모델 경로를 올바르게 지정합니다.
      emotionModelRef.current = await tf.loadLayersModel('web/model/facemo.json');
    };
  ...
  return (
    <div className="CalendarLayout">
      ...
      <div className="CameraBox">
        ...
        {emotion && <div className="EmotionResult">Detected Emotion: {emotion}</div>}
      </div>
    </div>
  );
}
